...
class alias
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'aliases WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'aliases WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function add()
	{
		...
		{*/
			$strquery='INSERT INTO '.DB_PREFIX.'aliases SET
						pi_id		= \''.$this->pi_id.'\',
						fname		= \''.$this->fname.'\',
						mname		= \''.$this->mname.'\',
						lname		= \''.$this->lname.'\',
	...
	function update()
	{
		...
		foreach($this->fname AS $key => $val)
		{
			if($this->id[$key] > 0)
			{
				$strquery='UPDATE '.DB_PREFIX.'aliases SET 
						pi_id		= \''.$this->pi_id.'\',
