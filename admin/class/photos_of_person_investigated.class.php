<?php
class photos_of_person_investigated
{
	//Property
	var $id;
	var $sp_id;
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'photos_of_person_investigated WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'photos_of_person_investigated WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function add()
	{
		...
		{
			$strquery='INSERT INTO '.DB_PREFIX.'photos_of_person_investigated SET
						sp_id		= \''.$this->sp_id.'\',
	...
	function update()
	{
		...
		foreach($this->fname AS $key => $val)
		{
			if($this->id[$key] > 0)
			{
				$strquery='UPDATE '.DB_PREFIX.'photos_of_person_investigated SET 
						sp_id		= \''.$this->sp_id.'\',
