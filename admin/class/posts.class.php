...
class posts
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'posts WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'posts WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function add()
	{
		...
		{
			$strquery='INSERT INTO '.DB_PREFIX.'posts SET
						sp_id		= \''.$this->sp_id.'\',
	...
	function update()
	{
		...
		foreach($this->fname AS $key => $val)
		{
			if($this->id[$key] > 0)
			{
				$strquery='UPDATE '.DB_PREFIX.'posts SET 
						sp_id		= \''.$this->sp_id.'\',
