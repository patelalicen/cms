...
class newspaper
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'newspaper WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'newspaper WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function add()
	{
		...
		$strquery='INSERT INTO '.DB_PREFIX.'newspaper SET
	...
	function update()
	{
		...
		$strquery='UPDATE '.DB_PREFIX.'newspaper SET 
