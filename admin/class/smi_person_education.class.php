...
class smi_person_education
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'smi_person_education WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'smi_person_education WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function setallvalues($id='',$condition='')
	{
		...
		while($ardoc= mysql_fetch_array($rs))
		{
			...
			$this->sp_id			= $ardoc['sp_id'];
						...
						$this->education_name[$i]		= $ardoc['education_name'];
	...
	function add()
	{
		...
		{
			$strquery='INSERT INTO '.DB_PREFIX.'smi_person_education SET
						sp_id		= \''.$this->sp_id.'\',
						
						
						education_name		= \''.$this->education_name[$key].'\',
						note		= \''.$this->note[$key].'\',
	...
	function update()
	{
		...
		foreach($this->fname AS $key => $val)
		{
			if($this->id[$key] > 0)
			{
				$strquery='UPDATE '.DB_PREFIX.'smi_person_education SET 
						sp_id		= \''.$this->sp_id.'\',
