...
class staff_with_clip
{
	...
	function fetchrecordset($id='',$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'staff_with_clip WHERE `is_deleted` = \'no\' ' . $condition . $order;
	...
	function fetchallasarray($intid=NULL, $stralphabet=NULL,$condition='',$order='id')
	{
		...
		$strquery='SELECT * FROM '.DB_PREFIX.'staff_with_clip WHERE `is_deleted` = \'no\' ' . $and . ' ORDER BY '.$order;
		$rs=mysql_query($strquery);
	...
	function setallvalues($id='',$condition='')
	{
		...
		while($ardoc= mysql_fetch_array($rs))
		{
			$this->id[$i]			= $ardoc['id'];
						...
						$this->ci_id[$i]		= $ardoc['ci_id'];
						$this->staff[$i]		= $ardoc['staff'];
	...
	function add()
	{
		...
		foreach($this->staff AS $key => $val)
		{
			if($this->id[$key] > 0)
			{
				$strquery='UPDATE '.DB_PREFIX.'staff_with_clip SET 
						
						case_id		= \''.$this->case_id.'\',
						ci_id		= \''.$this->ci_id[$key].'\',
			...
			{
				$strquery='INSERT INTO '.DB_PREFIX.'staff_with_clip SET
