function multiUploader(config){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				if(typeof this.items[i] != undefined){
					...
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="images/ajax-loader.gif" /></div><div id="'+uId+'_done" class="progress_done" style="display:none;"><img src="images/done.gif" /></div></div>';
				}
			}
	...
	multiUploader.prototype._uploader = function(file,f){
		if(typeof file[f] != undefined && self._validate(file[f].type) > 0){
			...
			data.append('hdnmode','drag_drop_images');
			data.append('gallery_id',$("#gallery_id").val());
				...
				success:function(rponse){
					...
					$.each(obj,function(k,fle){
						if($(fle).attr("rel") == rponse){
							...
							$(fle).find(".progress").hide();
							$(fle).find(".progress_done").show();
						}
					});
					if (f+1 < file.length) {
